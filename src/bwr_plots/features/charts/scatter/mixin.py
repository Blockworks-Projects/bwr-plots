...
class ScatterPlotMixin:
    def scatter_plot(
        ...
        current_fill_color = fill_color if fill_color is not None else cfg_plot["default_fill_color"]
        ...
        if effective_date is None:
            source_for_date = primary_data_orig if primary_data_orig is not None and not primary_data_orig.empty else secondary_data_orig
            if (
                ...
                try:
                    ...
                    print(f"[Warning] scatter_plot: Could not automatically determine max date: {e}. Using today's date.")
            else:
                effective_date = datetime.datetime.now().strftime("%Y-%m-%d")
        ...
        if primary_data_orig is not None and not primary_data_orig.empty:
            ...
            local_axis_options["primary_suffix"] = final_primary_suffix if final_primary_suffix is not None else ""
        ...
        if scaled_primary_data is not None:
            ...
            if y_values_for_range:
                yaxis_params = calculate_yaxis_grid_params(y_data=y_values_for_range, padding=0.05, num_gridlines=5)
        ...
        scaled_secondary_data = secondary_data_orig.copy() if secondary_data_orig is not None else None
        ...
        if scaled_secondary_data is not None and not scaled_secondary_data.empty:
            ...
            if auto_scale_y_values and pd.notna(max_value_secondary):
                scale_secondary, auto_suffix_secondary = _get_scale_and_suffix(max_value_secondary)

            if final_secondary_suffix is None:
                final_secondary_suffix = auto_suffix_secondary
            local_axis_options["secondary_suffix"] = final_secondary_suffix if final_secondary_suffix is not None else ""

            if scale_secondary > 1:
                try:
                    numeric_cols = scaled_secondary_data.select_dtypes(include=np.number).columns
                    scaled_secondary_data[numeric_cols] = scaled_secondary_data[numeric_cols] / scale_secondary
                except Exception as e:
        ...
        if xaxis_is_date:
            if scaled_primary_data is not None:
                ...
                if pd.api.types.is_datetime64_any_dtype(scaled_primary_data.index) and not scaled_primary_data.empty:
            ...
            if scaled_secondary_data is not None:
                ...
                if pd.api.types.is_datetime64_any_dtype(scaled_secondary_data.index) and not scaled_secondary_data.empty:
